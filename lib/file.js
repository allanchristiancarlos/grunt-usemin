  ...
  lines.forEach(function (l) {
    ...
    if (block && last) {
      ...
      if (asset && asset[2]) {
        ...
        var excludedFromSrc = grunt.config('useminPrepare').options.excludedFromSrc;

        for (var i = excludedFromSrc.length - 1; i >= 0; i--) {
          asset[2] = asset[2].replace(excludedFromSrc[i],'');
        };
